sap.ui.define([
	...
	return Controller.extend("sap.m.sample.Popover.C", {
		...
		onEditProfilePress: function (event) {
			this.EditProfile = new sap.ui.xmlfragment("GTree.fragment.EditProfile", _oController);
	...
	start.parents.reduce(function(previousId, currentId) {
		...
		if (previousParent) { _oController.plotConnector(previousParent, currentParent, 'partners'); }
 ...
 plotNode: function() {
	var person = arguments[0], personType = arguments[1], relative = arguments[2], numberOfParents = arguments[3], 
		node = _oController.get(person.id), relativeNode, element = {}, thisLevel, exists 
	;
	if (node) { return; }
...
plotConnector: function(source, destination, relation) {
  ...
  connector.id = comboId;
	...
	start = _oController.get(source.id); stop = _oController.get(destination.id);
    ...
    while (_oController.exists == _oController.detectConnectorCollision(connector)) { 
      connector.style.top = (_oController.parseInt(_oController.exists.style.top) + 4) + 'px'; 
    }
	}
	if (relation == 'parents') {
